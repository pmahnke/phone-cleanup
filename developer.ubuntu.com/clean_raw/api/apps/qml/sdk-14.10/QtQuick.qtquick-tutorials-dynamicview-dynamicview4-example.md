...
<p>Drag and drop isn't the only way items in a view can be re-ordered, using a VisualDataModel it is also possible to sort items based on model data. To do that we extend our VisualDataModel instance like this:</p>
...
<p>A sort is triggered whenever new items are added to the unsorted VisualDataGroup which we are notified of by the onChanged handler. If no sort function is currently selected we simply transfer all items from the unsorted group to the items group, otherwise we call sort with the selected sort function.</p>
...
<a class="prevPage" href="https://developer.ubuntu.comapps/qml/sdk-14.10/QtQuick.tutorials-dynamicview-dynamicview3/">QML Dynamic View Ordering Tutorial 3 - Moving Dragged Items</a>
</p>
