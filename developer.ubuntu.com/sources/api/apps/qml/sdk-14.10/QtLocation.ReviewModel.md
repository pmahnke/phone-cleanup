...
The ReviewModel is a read-only model used to fetch reviews about a [Place](../QtLocation.Place/index.html). The model incrementally fetches. The number of reviews which are fetched at a time is specified by the [batchSize](index.html#batchSize-prop) property. The total number of reviews available can be accessed via the [totalCount](index.html#totalCount-prop) property.
