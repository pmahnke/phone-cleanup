...
-   ****[inItems](index.html#inItems-prop)**** : int
-   ****[inPersistedItems](index.html#inPersistedItems-prop)**** : int
-   ****[isUnresolved](index.html#isUnresolved-prop)**** : bool
-   ****[itemsIndex](index.html#itemsIndex-prop)**** : int
-   ****[persistedItemsIndex](index.html#persistedItemsIndex-prop)**** : int
...
It is usually not necessary to create a DelegateModel. However, it can be useful for manipulating and accessing the [modelIndex](index.html#modelIndex-method) when a QAbstractItemModel subclass is used as the model. Also, DelegateModel is used together with [Package](../QtQml.Package/index.html) to provide delegates to multiple views, and with [DelegateModelGroup](../QtQml.DelegateModelGroup/index.html) to sort and filter delegate items.
...
An item can be removed from the persistedItems group by setting the [DelegateModel](index.html).[inPersistedItems](index.html#inPersistedItems-prop) property to false. If the item is not referenced by a view at that time it will be destroyed. Adding an item to this group will not create a new instance.
...
If the [model](index.html#model-prop) is a QAbstractItemModel subclass, the delegate can also reference a `hasModelChildren` property (optionally qualified by a *model*. prefix) that indicates whether the delegate's model item has any child nodes.
