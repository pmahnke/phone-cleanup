...
<td>Declares a type description file for the module that can be read by QML tools such as Qt Creator to access information about the types defined by the module's plugins. <code>&lt;File&gt;</code> is the (relative) file name of a <code>.qmltypes</code> file.
<p>Example:</p>
...
See examples/qml/plugins for an example that uses C++ plugins.
...
If the module is [installed](../QtQml.qtqml-modules-identifiedmodules/index.html) into the QML import path, clients could import and use the module in the following manner:
