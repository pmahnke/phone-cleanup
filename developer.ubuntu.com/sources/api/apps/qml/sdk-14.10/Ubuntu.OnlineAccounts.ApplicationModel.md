...
In the current implementation, the model is valid only if its [ApplicationModel::service](index.html#service-prop) property is set to a valid service ID.
...
-   `serviceUsage` is a description of how the application uses the service; this is set to a valid value only if the [ApplicationModel::service](index.html#service-prop) property is set to a valid service ID.
