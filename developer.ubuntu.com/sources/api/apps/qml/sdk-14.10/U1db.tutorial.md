...
A Database element is also a valuable tool for cases where specific data from a repository needs to be retrieved quickly, without indexing or querying ahead of time. The 'contents' object associated with the Database element can be used together with base items such as [TextField](../Ubuntu.Components.TextField/index.html) or [TextArea](../Ubuntu.Components.TextArea/index.html) and model-based items like ListView and [ListItems.Standard](../Ubuntu.Components.ListItems.Standard/index.html).
