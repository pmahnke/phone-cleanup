...
The follow example shows one [Rectangle](../QtQuick.Rectangle/index.html) tracking the position of another using SmoothedAnimation. The green rectangle's `x` and `y` values are bound to those of the red rectangle. Whenever these values change, the green rectangle smoothly animates to its new position:
