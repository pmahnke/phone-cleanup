...
QML [object types](../QtQml.qtqml-typesystem-objecttypes/index.html) defined in [QML documents](../QtQml.qtqml-documents-topic/index.html) can make use of JavaScript expressions which implement program logic. There are four ways that JavaScript can be used in a QML document:
...
-   [signal handlers](../QtQml.qtqml-syntax-objectattributes/index.html#signal-attributes) can be defined which are automatically evaluated when the object emits the associated signal
-   [custom methods](../QtQml.qtqml-syntax-objectattributes/index.html#method-attributes) can be defined in QML files as JavaScript functions
-   JavaScript files providing functions and variables can be [imported](../QtQml.qtqml-javascript-imports/index.html) in a QML document
...
It is occasionally necessary to run some imperative code at application (or component instance) startup. While it is tempting to just include the startup script as *global code* in an external script file, this can have severe limitations as the QML environment may not have been fully established. For example, some objects might not have been created or some [property bindings](../QtQml.qtqml-syntax-propertybinding/index.html) may not have been run. [JavaScript Environment Restrictions](../QtQml.qtqml-javascript-hostenvironment/index.html#javascript-environment-restrictions) covers the exact limitations of global script code.

Every QML object has an *attached* [Component](../QtQml.Component/index.html) property that references the component within which the object was instantiated. Every [Component](../QtQml.Component/index.html) will emit a `completed` signal, and thus every object can define an implementation for the `onCompleted()` handler which can be used to trigger the execution of script code at startup after the QML environment has been completely established. For example:
