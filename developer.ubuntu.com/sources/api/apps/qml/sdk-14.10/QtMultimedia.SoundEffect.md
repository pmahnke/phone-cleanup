...
This property holds the number of times the sound is played. A value of 0 or 1 means the sound will be played only once; set to [SoundEffect](index.html).Infinite to enable infinite looping.
...
This property contains the number of loops remaining before the sound effect stops by itself, or [SoundEffect](index.html).Infinite if that's what has been set in [loops](index.html#loops-prop).
...
This property holds the url for the sound to play. For the [SoundEffect](index.html) to attempt to load the source, the URL must exist and the application must have read permission in the specified directory. If the desired source is a local file the URL may be specified using either absolute or relative (to the file that declared the [SoundEffect](index.html)) pathing.
...
This property indicates the current status of the [SoundEffect](index.html) as enumerated within [SoundEffect](index.html). Possible statuses are listed below.

| Value                             | Description                                                                |
|-----------------------------------|----------------------------------------------------------------------------|
| [SoundEffect](index.html).Null    | No source has been set or the source is null.                              |
| [SoundEffect](index.html).Loading | The [SoundEffect](index.html) is trying to load the source.                |
| [SoundEffect](index.html).Ready   | The source is loaded and ready for play.                                   |
| [SoundEffect](index.html).Error   | An error occurred during operation, such as failure of loading the source. |
...
This is the default method for [SoundEffect](index.html).
