...
-   ****[onImportCompleted](index.html#onImportCompleted-signal)****()
...
This signal is emitted, when [OrganizerModel::importItems()](index.html#importItems-method) completes. The success of operation can be seen on *error* which is defined in [OrganizerModel::ImportError](index.html#ImportError-prop). *url* indicates the file, which was imported.

If the operation was succesful, items are now imported to backend. If [OrganizerModel::autoUpdate](index.html#autoUpdate-prop) is enabled, OrganizerModel::modelChanged will be emitted when imported items are also visible on [OrganizerModel](index.html)'s data model.
