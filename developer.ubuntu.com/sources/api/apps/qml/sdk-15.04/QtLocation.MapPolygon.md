...
The coordinates on the path cannot be directly changed after being added to the Polygon. Instead, copy the [path](index.html#path-prop) into a var, modify the copy and reassign the copy back to the [path](index.html#path-prop).
...
For drawing rectangles with "straight" edges (same latitude across one edge, same latitude across the other), the [MapRectangle](../QtLocation.MapRectangle/index.html) type provides a simpler, two-point API.

By default, the polygon is displayed as a 1 pixel black border with no fill. To change its appearance, use the [color](index.html#color-prop), [border.color](index.html#border.color-prop) and [border.width](index.html#border.width-prop) properties.
...
**See also** [addCoordinate](index.html#addCoordinate-method) and [removeCoordinate](index.html#removeCoordinate-method).
