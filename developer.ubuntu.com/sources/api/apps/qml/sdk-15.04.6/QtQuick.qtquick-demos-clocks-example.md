...
We use an [Image](https://developer.ubuntu.com/api/apps/qml/sdk-15.04.6/QtQuick.imageelements/#image) type to display arrows that indicate whether users can flick the view to see more clocks on the left or right:
...
We use [Image](https://developer.ubuntu.com/api/apps/qml/sdk-15.04.6/QtQuick.imageelements/#image) types within an [Item](../QtQuick.Item/index.html) type to display the time on an analog clock face. Different images are used for daytime and nighttime hours:
...
A [Rotation](../QtQuick.Rotation/index.html) transform applied to [Image](https://developer.ubuntu.com/api/apps/qml/sdk-15.04.6/QtQuick.imageelements/#image) types provides a way to rotate the clock hands. The `origin` property holds the point that stays fixed relative to the parent as the rest of the item rotates. The `angle` property determines the angle to rotate the hands in degrees clockwise.
