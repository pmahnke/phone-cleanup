...
The ListModel is a simple container of [ListElement](../QtQml.ListElement.md) definitions, each containing data roles. The contents can be defined dynamically, or explicitly in QML.
...
The content of a ListModel may be created and modified using the [clear()](#clear-method), [append()](#append-method), [set()](#set-method), [insert()](#insert-method) and [setProperty()](#setProperty-method) methods. For example:
...
ListModel can be used together with [WorkerScript](../QtQml.WorkerScript.md) access a list model from multiple threads. This is useful if list modifications are synchronous and take some time: the list operations can be moved to a different thread to avoid blocking of the main GUI thread.
...
**See also** Data Models, Threaded ListModel example, and [Qt QML](../QtQml.qtqml-index.md).
