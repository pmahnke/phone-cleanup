...
You can use [VideoOutput](../QtMultimedia.VideoOutput.md) to render content that is provided by either a [MediaPlayer](../QtMultimedia.MediaPlayer.md) or a [Camera](../QtMultimedia.Camera.md). The [VideoOutput](../QtMultimedia.VideoOutput.md) is a visual component that can be transformed or acted upon by shaders (as the [QML Video Shader Effects Example](https://developer.ubuntu.comapps/qml/sdk-14.10/QtMultimedia.video-qmlvideofx/) shows), while all media decoding and playback control is handled by the [MediaPlayer](../QtMultimedia.MediaPlayer.md).

Alternatively there is also a higher level [Video](../QtMultimedia.Video.md) type that acts as a single, visual element to play video and control playback.
