...
-   ****[acceptLangs](#acceptLangs-prop)**** : string
-   ****[cachePath](#cachePath-prop)**** : url
-   ****[dataPath](#dataPath-prop)**** : url
-   ****[userAgent](#userAgent-prop)**** : string
...
Note that the [WebView](../Ubuntu.Web.WebView.md) component already uses this default context, so there is no need to explicitly instantiate a `WebContext` unless you want to override some of its default properties.
...
The local path where network data will be cached. If not set, it defaults to [dataPath](#dataPath-prop).
