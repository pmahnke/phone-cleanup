...
Place objects are typically obtained from a search model and will generally only have their basic properties set. The [detailsFetched](#detailsFetched-prop) property can be used to test if further property values need to be fetched from the [Plugin](../QtLocation.Plugin.md). This can be done by invoking the [getDetails()](#getDetails-method) method. Progress of the fetching operation can be monitored with the [status](#status-prop) property, which will be set to Place.Fetching when the details are being fetched.
...
Each place is assigned zero or more [categories](#categories-prop). Categories are typically used when searching for a particular kind of place, such as a restaurant or hotel. Some places have a [ratings](#ratings-prop) object, which gives an indication of the quality of the place.

Place metadata is provided by a [supplier](#supplier-prop) who may require that an [attribution](#attribution-prop) message be displayed to the user when the place details are viewed.
...
Places may have additional attributes which are not covered in the formal API. The [extendedAttributes](#extendedAttributes-prop) property provides access to these. The type of extended attributes available is specific to each [Plugin](../QtLocation.Plugin.md).

A Place is almost always tied to a [plugin](#plugin-prop). The [plugin](#plugin-prop) property must be set before it is possible to call [save()](#save-method), [remove()](#remove-method) or [getDetails()](#getDetails-method). The [reviewModel](#reviewModel-prop), [imageModel](#imageModel-prop) and [editorialModel](#editorialModel-prop) are only valid then the [plugin](#plugin-prop) property is set.

<span id="saving-a-place"></span>
### Saving a Place

If the [Plugin](../QtLocation.Plugin.md) supports it, the Place type can be used to save a place. First create a new Place and set its properties:
...
This property holds the list of categories this place is a member of. The categories that can be assigned to a place are specific to each [plugin](#plugin-prop).
...
This property holds the unique identifier of the place. The place identifier is only meaningful to the [Plugin](../QtLocation.Plugin.md) that generated it and is not transferable between [Plugins](../QtLocation.Plugin.md). The place id is not guaranteed to be universally unique, but unique within the [Plugin](../QtLocation.Plugin.md) that generated it.

If only the place identifier is known, all other place data can fetched from the [Plugin](../QtLocation.Plugin.md).
...
This property holds the [Plugin](../QtLocation.Plugin.md) that provided this place which can be used to retrieve more information about the service.
...
|                             |                                                                                                                                           |
|-----------------------------|-------------------------------------------------------------------------------------------------------------------------------------------|
| Place.UnspecifiedVisibility | The visibility of the place is unspecified, the default visibility of the [Plugin](../QtLocation.Plugin.md) will be used.         |
