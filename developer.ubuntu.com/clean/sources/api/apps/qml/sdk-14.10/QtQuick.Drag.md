...
-   ****[dragType](#dragType-prop)**** : enumeration
-   ****[hotSpot](#hotSpot-prop)**** : QPointF
-   ****[keys](#keys-prop)**** : stringlist
-   ****[mimeData](#mimeData-prop)**** : stringlist
-   ****[proposedAction](#proposedAction-prop)**** : enumeration
-   ****[source](#source-prop)**** : Object
...
When a drag is [active](#active-prop) on an item any change in that item's position will generate a drag event that will be sent to any [DropArea](../QtQuick.DropArea.md) that intersects with the new position of the item. Other items which implement drag and drop event handlers can also receive these events.

The following snippet shows how an item can be dragged with a [MouseArea](../QtQuick.MouseArea.md). However, dragging is not limited to mouse drags, anything that can move an item can generate drag events, this can include touch events, animations and bindings.
...
Binding this property to the active property of MouseArea::drag will cause [startDrag](#startDrag-method) to be called when the user starts dragging.
...
When using `Drag.Automatic` you should also define [mimeData](#mimeData-prop) and bind the [active](#active-prop) property to the active property of MouseArea.drag.
...
This signal is emitted when a drag finishes and the drag was started with the [startDrag()](#startDrag-method) method or started automatically using the [dragType](#dragType-prop) property.
...
This signal is emitted when a drag is started with the [startDrag()](#startDrag-method) method or when it is started automatically using the [dragType](#dragType-prop) property.
...
The optional *supportedActions* argument can be used to override the [supportedActions](#supportedActions-prop) property for the started sequence.
