...
If a MouseArea overlaps with the area of other MouseArea items, you can choose to propagate `clicked`, `doubleClicked` and `pressAndHold` events to these other items by setting [propagateComposedEvents](#propagateComposedEvents-prop) to true and rejecting events that should be propagated. See the [propagateComposedEvents](#propagateComposedEvents-prop) documentation for details.

By default, MouseArea items only report mouse clicks and not changes to the position of the mouse cursor. Setting the [hoverEnabled](#hoverEnabled-prop) property ensures that handlers defined for onPositionChanged, onEntered and onExited are used and that the [containsMouse](#containsMouse-prop) property is updated even when no mouse buttons are pressed.

<span id="example-usage"></span>
Example Usage
-------------

![](https://developer.ubuntu.com/static/devportal_uploaded/3a8460d3-688b-4ea2-bbb8-611bf25da062-api/apps/qml/sdk-14.10/QtQuick.MouseArea/images/qml-mousearea-snippet.png)

The following example uses a MouseArea in a [Rectangle](../QtQuick.Rectangle.md) that changes the [Rectangle](../QtQuick.Rectangle.md) color to red when clicked:
...
Many MouseArea signals pass a [mouse](../QtQuick.MouseEvent.md) parameter that contains additional information about the mouse event, such as the position, button, and any key modifiers.
...
**See also** [MouseEvent](../QtQuick.MouseEvent.md), MouseArea example, and [Important Concepts In Qt Quick - User Input](../QtQuick.qtquick-input-topic.md).
...
This signal is for advanced use: it is useful when there is more than one [MouseArea](index.html) that is handling input, or when there is a [MouseArea](index.html) inside a [Flickable](../QtQuick.Flickable.md). In the latter case, if you execute some logic in the `onPressed` signal handler and then start dragging, the [Flickable](../QtQuick.Flickable.md) will steal the mouse handling from the [MouseArea](index.html). In these cases, to reset the logic when the [MouseArea](index.html) has lost the mouse handling to the [Flickable](../QtQuick.Flickable.md), `canceled` should be handled in addition to [released](#released-signal).
