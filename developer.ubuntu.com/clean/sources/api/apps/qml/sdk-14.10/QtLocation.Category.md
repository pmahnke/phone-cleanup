...
If the [Plugin](../QtLocation.Plugin.md) supports it, categories can be created or removed. To create a new category construct a new Category object and set its properties, then invoke the [save()](#save-method) method.
...
To remove a category ensure that the [plugin](#plugin-prop) and [categoryId](#categoryId-prop) properties are set and call the [remove()](#remove-method) method.
...
This property holds the identifier of the category. The categoryId is a string which uniquely identifies this category within the categories [plugin](#plugin-prop).
...
Note that visibility does not affect how [Place](../QtLocation.Place.md)s associated with the category are displayed in the user-interface of an application on the device. Instead, it defines the sharing semantics of the category.
