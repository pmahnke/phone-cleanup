...
A [MapGestureArea](index.html) is automatically created with a new Map and available with the [gesture](../QtLocation.Map.md#gesture-prop) property. This is the only way to create a [MapGestureArea](index.html), and once created this way cannot be destroyed without its parent Map.

The two most commonly used properties of the [MapGestureArea](index.html) are the [enabled](#enabled-prop) and [activeGestures](#activeGestures-prop) properties. Both of these must be set before a [MapGestureArea](index.html) will have any effect upon interaction with the Map. The [flickDeceleration](#flickDeceleration-prop) property controls how quickly the map pan slows after contact is released while panning the map.
