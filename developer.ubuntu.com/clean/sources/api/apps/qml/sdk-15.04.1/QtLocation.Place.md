...
-   ****[attribution](#attribution-prop)**** : string
-   ****[categories](#categories-prop)**** : list&lt;Category&gt;
-   ****[contactDetails](#contactDetails-prop)**** : ContactDetails
-   ****[detailsFetched](#detailsFetched-prop)**** : bool
-   ****[editorialModel](#editorialModel-prop)**** : EditorialModel
-   ****[extendedAttributes](#extendedAttributes-prop)**** : ExtendedAttributes
-   ****[favorite](#favorite-prop)**** : Place
-   ****[icon](#icon-prop)**** : Icon
-   ****[imageModel](#imageModel-prop)**** : ImageModel
-   ****[location](#location-prop)**** : QtPositioning::Location
-   ****[name](#name-prop)**** : string
-   ****[place](#place-prop)**** : QPlace
-   ****[placeId](#placeId-prop)**** : string
-   ****[plugin](#plugin-prop)**** : Plugin
-   ****[primaryEmail](#primaryEmail-prop)**** : string
-   ****[primaryFax](#primaryFax-prop)**** : string
-   ****[primaryPhone](#primaryPhone-prop)**** : string
-   ****[primaryWebsite](#primaryWebsite-prop)**** : string
-   ****[ratings](#ratings-prop)**** : Ratings
-   ****[reviewModel](#reviewModel-prop)**** : ReviewModel
-   ****[status](#status-prop)**** : enumeration
-   ****[supplier](#supplier-prop)**** : Supplier
...
-   ****[copyFrom](#copyFrom-method)****(Place *original*)
-   string ****[errorString](#errorString-method)****()
-   void ****[getDetails](#getDetails-method)****()
-   ****[initializeFavorite](#initializeFavorite-method)****(Plugin *destinationPlugin*)
...
The Place type represents a physical location with additional metadata describing that location. Contrasted with Location, Address, and coordinate type which are used to describe where a location is. The basic properties of a Place are its [name](#name-prop) and [location](#location-prop).

Place objects are typically obtained from a search model and will generally only have their basic properties set. The [detailsFetched](#detailsFetched-prop) property can be used to test if further property values need to be fetched from the [Plugin](../QtLocation.location-places-qml.md#plugin). This can be done by invoking the [getDetails()](#getDetails-method) method. Progress of the fetching operation can be monitored with the [status](#status-prop) property, which will be set to Place.Fetching when the details are being fetched.

The Place type has many properties holding information about the location. Details on how to contact the place are available from the [contactDetails](#contactDetails-prop) property. Convenience properties for obtaining the primary [phone](#primaryPhone-prop), [fax](#primaryFax-prop), [email](#primaryEmail-prop) and [website](#primaryWebsite-prop) are also available.

Each place is assigned zero or more [categories](../QtLocation.location-places-backend.md#categories). Categories are typically used when searching for a particular kind of place, such as a restaurant or hotel. Some places have a [ratings](#ratings-prop) object, which gives an indication of the quality of the place.
...
Places have an associated [icon](#icon-prop) which can be used to represent a place on a map or to decorate a delegate in a view.

Places may have additional rich content associated with them. The currently supported rich content include editorial descriptions, reviews and images. These are exposed as a set of models for retrieving the content. Editorial descriptions of the place are available from the [editorialModel](#editorialModel-prop) property. Reviews of the place are available from the [reviewModel](#reviewModel-prop) property. A gallery of pictures of the place can be accessed using the [imageModel](#imageModel-prop) property.

Places may have additional attributes which are not covered in the formal API. The [extendedAttributes](#extendedAttributes-prop) property provides access to these. The type of extended attributes available is specific to each [Plugin](../QtLocation.location-places-qml.md#plugin).

A Place is almost always tied to a [plugin](../QtLocation.location-places-qml.md#plugin). The [plugin](../QtLocation.location-places-qml.md#plugin) property must be set before it is possible to call [save()](#save-method), [remove()](#remove-method) or [getDetails()](#getDetails-method). The [reviewModel](#reviewModel-prop), [imageModel](#imageModel-prop) and [editorialModel](#editorialModel-prop) are only valid then the [plugin](../QtLocation.location-places-qml.md#plugin) property is set.

<span id="saving-a-place"></span>
### Saving a Place

If the [Plugin](../QtLocation.location-places-qml.md#plugin) supports it, the Place type can be used to save a place. First create a new Place and set its properties:
...
Then invoke the [save()](#save-method) method:

``` qml
    myPlace.save();
```

The [status](#status-prop) property will change to Place.Saving and then to Place.Ready if the save was successful or to Place.Error if an error occurs.

If the [placeId](#placeId-prop) property is set, the backend will update an existing place otherwise it will create a new place. On success the [placeId](#placeId-prop) property will be updated with the identifier of the newly saved place.
...
It is generally recommended that saving across plugins be handled as saving [favorites](#favorites) as explained in the Favorites section. However there is another approach which is to create a new place, set its (destination) plugin and then use the [copyFrom()](#copyFrom-method) method to copy the details of the original place. Using [copyFrom()](#copyFrom-method) only copies data that is supported by the destination plugin, plugin specific data such as the place identifier is not copied over. Once the copy is done, the place is in a suitable state to be saved.

The following snippet provides an example of saving a place to a different plugin using the [copyFrom](#copyFrom-method) method:
...
To remove a place, ensure that a Place object with a valid [placeId](#placeId-prop) property exists and call its [remove()](#remove-method) method. The [status](#status-prop) property will change to Place.Removing and then to Place.Ready if the save was successful or to Place.Error if an error occurs.
...
The [PlaceSearchModel](../QtLocation.PlaceSearchModel.md) has a favoritesPlugin property. If the property is set, any places found during a search are checked against the favoritesPlugin to see if there is a corresponding favorite place. If so, the favorite property of the Place is set, otherwise the favorite property is remains null.

**See also** [PlaceSearchModel](../QtLocation.PlaceSearchModel.md).
...
This property holds the list of categories this place is a member of. The categories that can be assigned to a place are specific to each [plugin](../QtLocation.location-places-qml.md#plugin).
...
<tbody>
<tr class="odd">
<td><p><span id="contactDetails-prop"></span><span class="name">contactDetails</span> : <span class="type"><a href="QtLocation.ContactDetails.md">ContactDetails</a></span></p></td>
</tr>
</tbody>
</table>

This property holds the contact information for this place, for example a phone number or a website URL. This property is a map of [ContactDetail](../QtLocation.ContactDetail.md) objects.
...
This property indicates whether the details of the place have been fetched. If this property is false, the place details have not yet been fetched. Fetching can be done by invoking the [getDetails()](#getDetails-method) method.

**See also** [getDetails()](#getDetails-method).
...
<tbody>
<tr class="odd">
<td><p><span id="editorialModel-prop"></span><span class="name">editorialModel</span> : <span class="type"><a href="QtLocation.EditorialModel.md">EditorialModel</a></span></p></td>
</tr>
</tbody>
</table>
...
<tbody>
<tr class="odd">
<td><p><span id="extendedAttributes-prop"></span><span class="name">extendedAttributes</span> : <span class="type"><a href="QtLocation.ExtendedAttributes.md">ExtendedAttributes</a></span></p></td>
</tr>
</tbody>
</table>
...
<tbody>
<tr class="odd">
<td><p><span id="icon-prop"></span><span class="name">icon</span> : <span class="type"><a href="QtLocation.Icon.md">Icon</a></span></p></td>
</tr>
</tbody>
</table>
...
<tbody>
<tr class="odd">
<td><p><span id="imageModel-prop"></span><span class="name">imageModel</span> : <span class="type"><a href="QtLocation.ImageModel.md">ImageModel</a></span></p></td>
</tr>
</tbody>
</table>
...
This property holds the unique identifier of the place. The place identifier is only meaningful to the [Plugin](../QtLocation.location-places-qml.md#plugin) that generated it and is not transferable between [Plugins](../QtLocation.location-places-qml.md#plugin). The place id is not guaranteed to be universally unique, but unique within the [Plugin](../QtLocation.location-places-qml.md#plugin) that generated it.

If only the place identifier is known, all other place data can fetched from the [Plugin](../QtLocation.location-places-qml.md#plugin).
...
This property holds the [Plugin](../QtLocation.location-places-qml.md#plugin) that provided this place which can be used to retrieve more information about the service.
...
<tbody>
<tr class="odd">
<td><p><span id="ratings-prop"></span><span class="name">ratings</span> : <span class="type"><a href="QtLocation.Ratings.md">Ratings</a></span></p></td>
</tr>
</tbody>
</table>
...
<tbody>
<tr class="odd">
<td><p><span id="reviewModel-prop"></span><span class="name">reviewModel</span> : <span class="type"><a href="QtLocation.ReviewModel.md">ReviewModel</a></span></p></td>
</tr>
</tbody>
</table>
...
<tbody>
<tr class="odd">
<td><p><span id="supplier-prop"></span><span class="name">supplier</span> : <span class="type"><a href="QtLocation.Supplier.md">Supplier</a></span></p></td>
</tr>
</tbody>
</table>
...
|                             |                                                                                                                                                       |
|-----------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------|
| Place.UnspecifiedVisibility | The visibility of the place is unspecified, the default visibility of the [Plugin](../QtLocation.location-places-qml.md#plugin) will be used. |
...
The [status](#status-prop) property will change to Place.Fetching while the fetch is in progress. On success the object's properties will be updated, [status](#status-prop) will be set to Place.Ready and [detailsFetched](#detailsFetched-prop) will be set to true. On error [status](#status-prop) will be set to Place.Error. The [errorString()](#errorString-method) method can be used to get the details of the error.
...
<tbody>
<tr class="odd">
<td><p><span id="initializeFavorite-method"></span><span class="name">initializeFavorite</span>(<span class="type"><a href="QtLocation.Plugin.md">Plugin</a></span> <em>destinationPlugin</em>)</p></td>
</tr>
</tbody>
</table>
...
The [status](#status-prop) property will change to Place.Removing while the save operation is in progress. On success [status](#status-prop) will be set to Place.Ready. On error [status](#status-prop) will be set to Place.Error. The [errorString()](#errorString-method) method can be used to get the details of the error.
...
The [status](#status-prop) property will change to Place.Saving while the save operation is in progress. On success the [status](#status-prop) will be set to Place.Ready. On error [status](#status-prop) will be set to Place.Error. The [errorString()](#errorString-method) method can be used to get the details of the error.

If the [placeId](#placeId-prop) property was previously empty, it will be assigned a valid value automatically during a successful save operation.

Note that a [PlaceSearchModel](../QtLocation.PlaceSearchModel.md) will call [Place::getDetails](#getDetails-method) on any place that it detects an update on. A consequence of this is that whenever a Place from a [PlaceSearchModel](../QtLocation.PlaceSearchModel.md) is successfully saved, it will be followed by a fetch of place details, leading to a sequence of state changes of `Saving`, `Ready`, `Fetching`, `Ready`.
